...
This is a refactored version of [zennit-crp](https://github.com/rachtibat/zennit-crp), redesigned to
align with [zennit](https://github.com/chr5tphr/zennit)'s architecture (composites, hooks, attributors as
context managers). All original functionality is preserved with a modernized API.

## Installation

Install from a cloned repository using [uv](https://docs.astral.sh/uv/):
...
Check out the [jupyter notebook tutorials.](tutorials/) Please start with attribution and then feature_visualization.
...
# each channel is defined as a concept (or define your own notion!)
cc = ChannelConcept()

# get layer names of Conv2D and MLP layers
layer_names = get_layer_names(model, [nn.Conv2d, nn.Linear])

# use ConditionalGradient as a context manager
with ConditionalGradient(model, composite=composite) as attributor:
    # conditional attribution for channel 50 in layer features.27 wrt. output 1
    conditions = [{"features.27": [50], "y": [1]}]

    attr = attributor(data, conditions, record_layers=layer_names)
...
from zennit_crp.image import plot_grid

# define which concept is used in each layer
layer_map = {name: cc for name in layer_names}

# compute visualization (takes ~30 min for VGG16 + ImageNet on Titan RTX)
fv = FeatureVisualization(attributor, dataset, layer_map)
fv.run(composite, 0, len(dataset))

# visualize MaxRelevance reference images for top-5 concepts
ref_c = fv.get_max_reference(
    concept_ids[:5], "features.40", "relevance", composite=composite,
)

plot_grid(ref_c)
```

### Exclude Parallel (Shortcut Connections)

```python
# restrict gradient flow to avoid shortcut connections
conditions = [{"layer4.1.conv2": [10], "y": [3]}]
...
```python
# efficiently generate attributions for many conditions (single forward pass)
conditions = [{"conv1": [i], "y": [0]} for i in range(64)]

with ConditionalGradient(model, composite=composite) as attributor:
    for batch_attr in attributor.generate(data, conditions, batch_size=16):
        process(batch_attr.heatmap, batch_attr.relevances)
```
...
from zennit_crp.graph import trace_model_graph

# trace model to find layer connectivity
model_graph = trace_model_graph(model, sample, layer_names)

# decompose a concept into lower-level concepts
with ConditionalGradient(model, composite=composite) as attributor:
    ag = AttributionGraph(attributor, model_graph, layer_map)
    result = ag(sample, concept_id=5, layer_name="features.27", target=1)
    result.nodes, result.connections
```

## API Reference

### Key Differences from Original zennit-crp

| Original (`crp.`) | Refactored (`zennit_crp.`) | Notes |
|---|---|---|
| `CondAttribution(model)` | `ConditionalGradient(model, composite=composite)` | Extends zennit's `Gradient`; used as context manager |
| `attr(data, conds, composite, record_layer=...)` | `attr(data, conds, record_layers=..., ...)` | Keyword-only args after conditions; composite passed at construction |
| `attr.generate(..., mask_map=..., batch_size=...)` | `attr.generate(..., mask_fn=..., batch_size=...)` | `mask_fn` can be callable or dict of per-layer callables |
| `MaskHook` (standalone hook) | `MaskHook(Hook)` | Extends zennit's `Hook` base class |
| `rules.py` | `hooks.py` | Renamed module for clarity |

## Contributing

We use [ruff](https://docs.astral.sh/ruff/) for formatting and linting, and
[numpydoc](https://numpydoc.readthedocs.io/en/latest/format.html) for docstrings.
